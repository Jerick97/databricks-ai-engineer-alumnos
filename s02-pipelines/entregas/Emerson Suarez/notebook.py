...
# MAGIC %md
# MAGIC ## 4.1 · Reconstruir Silver después de la demostración de CDF
# MAGIC
# MAGIC   La demostración de Change Data Feed modificó manualmente el campo `Descuento`, pero no recalculó `ingreso_linea`. Esto dejó ambas columnas
# MAGIC temporalmente desalineadas. Volvemos a ejecutar la sección 1 para reconstruir `silver.detalles_pedidos` desde Bronze y aplicar nuevamente la
# MAGIC regla de negocio completa: el ingreso de cada línea debe calcularse usando su descuento actual.
# MAGIC
# MAGIC   Después vuelve a ejecutar la celda completa de la sección 1.

# COMMAND ----------

# MAGIC %md
# MAGIC ## 4.2 · Confirmar que Change Data Feed continúa habilitado
# MAGIC  
# MAGIC   Al reconstruir la tabla Silver queremos asegurarnos de que Change Data Feed siga activo. Esta propiedad permitirá que procesos posteriores
# MAGIC identifiquen qué filas fueron insertadas, actualizadas o eliminadas sin tener que comparar la tabla completa. También será necesaria para    
# MAGIC mantener sincronizado el índice de Vector Search de la sesión 4.
...
# MAGIC %md
# MAGIC ## 4.3 · Verificar la coherencia de la regla de negocio
# MAGIC
# MAGIC   Antes de construir Gold comprobamos que `ingreso_linea` coincida con la fórmula oficial de ventas para todas las filas. Esta validación
# MAGIC evita publicar métricas derivadas de una tabla inconsistente. El resultado debe ser cero: cualquier valor mayor indica que el descuento y el
# MAGIC ingreso calculado no corresponden entre sí.
...
# MAGIC %md
# MAGIC ### Entregable 1: confirmar el pipeline incremental
...
# MAGIC %md
# MAGIC ### Entregable 2: regla de calidad propia
...
# MAGIC %md
# MAGIC ### Entregable 3: tabla Gold propia
...
# MAGIC %md
# MAGIC #### Verifica el resultado y el comentario
